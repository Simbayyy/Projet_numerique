...
def compute_energy(pos_1,pos_2,charge_1,charge_2):
    ...
    """
    return charge_1*charge_2/(np.linalg.norm(pos_1-pos_2))**2
...
@jit(nopython = True)
def compute_residue_energy(position_1, position_2, charges_1, charges_2):
    """
    Compute sum of coulombic forces for all pairs of particles in two residues.
    ...
    for i in range(len(position_1)):
        for j in range(len(position_2)):
            force += compute_energy(position_1[i],position_2[j],charges_1[i],charges_2[j])
    return force